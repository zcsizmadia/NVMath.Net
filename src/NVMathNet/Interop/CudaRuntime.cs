...
{
    // ── Library names ────────────────────────────────────────────────────────
    private const string LibWindows = "cudart64_13.dll";
    private const string LibLinux   = "libcudart.so.13";
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CudaError cudaDeviceCanAccessPeerDelegate(int* canAccessPeer, int device, int peerDevice);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CudaError cudaDeviceEnablePeerAccessDelegate(int peerDevice, uint flags);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CudaError cudaDeviceDisablePeerAccessDelegate(int peerDevice);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CudaError cudaMemcpyPeerAsyncDelegate(
        void* dst, int dstDevice, void* src, int srcDevice, nuint count, nint stream);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CudaError cudaGetDevicePropertiesDelegate(byte* prop, int device);
    ...
    private static readonly Lazy<cudaDeviceCanAccessPeerDelegate>    _canAccessPeer    = new(() => Load<cudaDeviceCanAccessPeerDelegate>("cudaDeviceCanAccessPeer"));
    private static readonly Lazy<cudaDeviceEnablePeerAccessDelegate> _enablePeerAccess = new(() => Load<cudaDeviceEnablePeerAccessDelegate>("cudaDeviceEnablePeerAccess"));
    private static readonly Lazy<cudaDeviceDisablePeerAccessDelegate> _disablePeerAccess = new(() => Load<cudaDeviceDisablePeerAccessDelegate>("cudaDeviceDisablePeerAccess"));
    private static readonly Lazy<cudaMemcpyPeerAsyncDelegate>       _memcpyPeerAsync  = new(() => Load<cudaMemcpyPeerAsyncDelegate>("cudaMemcpyPeerAsync"));
    private static readonly Lazy<cudaGetDevicePropertiesDelegate>   _getDeviceProps   = new(() => Load<cudaGetDevicePropertiesDelegate>("cudaGetDeviceProperties"));
    ...
    /// <summary>Returns whether <paramref name="device"/> can access memory on <paramref name="peerDevice"/>.</summary>
    public static bool DeviceCanAccessPeer(int device, int peerDevice)
    {
        int result;
        CheckError(_canAccessPeer.Value(&result, device, peerDevice), "cudaDeviceCanAccessPeer");
        return result != 0;
    }

    /// <summary>
    /// Enables peer access from the current device to <paramref name="peerDevice"/>.
    /// Must be called after <see cref="SetDevice"/> to set the current device.
    /// </summary>
    public static void DeviceEnablePeerAccess(int peerDevice) =>
        CheckError(_enablePeerAccess.Value(peerDevice, 0), "cudaDeviceEnablePeerAccess");

    /// <summary>Disables previously enabled peer access to <paramref name="peerDevice"/>.</summary>
    public static void DeviceDisablePeerAccess(int peerDevice) =>
        CheckError(_disablePeerAccess.Value(peerDevice), "cudaDeviceDisablePeerAccess");

    /// <summary>
    /// Copies <paramref name="count"/> bytes from <paramref name="srcDevice"/> to <paramref name="dstDevice"/>
    /// asynchronously on <paramref name="stream"/>.
    /// </summary>
    public static void MemcpyPeerAsync(void* dst, int dstDevice, void* src, int srcDevice, nuint count, nint stream) =>
        CheckError(_memcpyPeerAsync.Value(dst, dstDevice, src, srcDevice, count, stream), "cudaMemcpyPeerAsync");

    /// <summary>
    /// Returns the device name for the given device ordinal.
    /// Reads the first 256 bytes of the cudaDeviceProp structure (name field).
    /// </summary>
    public static string GetDeviceName(int device)
    {
        // cudaDeviceProp is large (~1024 bytes); name is the first 256 bytes (char[256])
        byte* prop = stackalloc byte[1024];
        CheckError(_getDeviceProps.Value(prop, device), "cudaGetDeviceProperties");
        return Marshal.PtrToStringAnsi((nint)prop) ?? $"Device {device}";
    }

    /// <summary>
    /// Gets the CUDA data type corresponding to the specified type parameter.
    /// </summary>
    /// <typeparam name="T">The type to get the CUDA data type for.</typeparam>
    /// <returns>The CUDA data type corresponding to the specified type.</returns>
    public static CudaDataType GetCudaDataType<T>() where T : unmanaged, INumberBase<T> 
        => GetCudaDataType(typeof(T));

    /// <summary>
    /// Gets the CUDA data type corresponding to the specified <see cref="Type"/>.
    /// </summary>
    /// <param name="t"></param>
    /// <returns></returns>
    /// <exception cref="NotSupportedException"></exception>
    public static CudaDataType GetCudaDataType(Type t)
    {
        if (t == typeof(float))
        {
            return CudaDataType.R_32F;
        }
        else if (t == typeof(double))
        {
            return CudaDataType.R_64F;
        }
        else if (t == typeof(Half))
        {
            return CudaDataType.R_16F;
        }

        throw new NotSupportedException($"Unsupported element type {t} for sparse matrix.");
    }
}
