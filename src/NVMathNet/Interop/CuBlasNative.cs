...
/// <summary>Side mode for cuBLAS structured operations.</summary>
public enum CuBlasSideMode : int
{
    /// <summary>Matrix A is on the left: C = alpha*A*B + beta*C.</summary>
    Left = 0,
    /// <summary>Matrix A is on the right: C = alpha*B*A + beta*C.</summary>
    Right = 1,
}

/// <summary>Diagonal type for triangular matrices.</summary>
public enum CuBlasDiagType : int
{
    /// <summary>Diagonal elements are not assumed to be one.</summary>
    NonUnit = 0,
    /// <summary>Diagonal elements are assumed to be one (unit triangular).</summary>
    Unit = 1,
}
...
public enum CuBlasLtEpilogue : int
{
    ...
    GeluBias        = 36,      // GELU | BIAS
    /// <summary>Apply ReLU and save auxiliary data for backward pass.</summary>
    ReluAux         = 130,     // RELU | 128
    /// <summary>Apply ReLU with bias and save auxiliary data.</summary>
    ReluAuxBias     = 134,     // RELU_AUX | BIAS
    /// <summary>Backward ReLU — compute gradient through ReLU.</summary>
    DreLu           = 136,     // 8 | 128
    /// <summary>Backward ReLU with bias gradient.</summary>
    DreLuBGrad      = 152,     // DRELU | 16
    /// <summary>Apply GELU and save auxiliary data for backward pass.</summary>
    GeluAux         = 160,     // GELU | 128
    /// <summary>Apply GELU with bias and save auxiliary data.</summary>
    GeluAuxBias     = 164,     // GELU_AUX | BIAS
    /// <summary>Backward GELU — compute gradient through GELU.</summary>
    DGelu           = 192,     // 64 | 128
    /// <summary>Backward GELU with bias gradient.</summary>
    DGeluBGrad      = 208,     // DGELU | 16
    /// <summary>Backward bias-gradient accumulation for A.</summary>
    BGradA          = 256,
    /// <summary>Backward bias-gradient accumulation for B.</summary>
    BGradB          = 512,
}
...
{
    private const string LibWindows    = "cublas64_13.dll";
    private const string LibLinux      = "libcublas.so.13";
    private const string LibLtWindows  = "cublasLt64_13.dll";
    private const string LibLtLinux    = "libcublasLt.so.13";
    ...
    // ── Delegate types (symmetric/triangular) ─────────────────────────────────

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasSsymmDelegate(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        int m, int n,
        float* alpha, float* a, int lda,
        float* b, int ldb,
        float* beta, float* c, int ldc);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasDsymmDelegate(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        int m, int n,
        double* alpha, double* a, int lda,
        double* b, int ldb,
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasStrsmDelegate(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasDtrsmDelegate(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        CuBlasOperation trans, CuBlasDiagType diag,
        int m, int n,
        double* alpha, double* a, int lda,
    ...
    // ── Delegate types (diagonal multiply) ───────────────────────────────────

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasSdgmmDelegate(
        nint handle, CuBlasSideMode mode,
        int m, int n,
        float* a, int lda,
        float* x, int incx,
        float* c, int ldc);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate CuBlasStatus cublasDdgmmDelegate(
        nint handle, CuBlasSideMode mode,
        int m, int n,
        double* a, int lda,
        double* x, int incx,
    ...
    private static readonly Lazy<cublasSsymmDelegate>                      _ssymm           = new(() => Load<cublasSsymmDelegate>(LibName, "cublasSsymm_v2"));
    private static readonly Lazy<cublasDsymmDelegate>                      _dsymm           = new(() => Load<cublasDsymmDelegate>(LibName, "cublasDsymm_v2"));
    private static readonly Lazy<cublasStrsmDelegate>                      _strsm           = new(() => Load<cublasStrsmDelegate>(LibName, "cublasStrsm_v2"));
    private static readonly Lazy<cublasDtrsmDelegate>                      _dtrsm           = new(() => Load<cublasDtrsmDelegate>(LibName, "cublasDtrsm_v2"));
    private static readonly Lazy<cublasSdgmmDelegate>                      _sdgmm           = new(() => Load<cublasSdgmmDelegate>(LibName, "cublasSdgmm"));
    private static readonly Lazy<cublasDdgmmDelegate>                      _ddgmm           = new(() => Load<cublasDdgmmDelegate>(LibName, "cublasDdgmm"));
    ...
    /// <summary>
    /// Single-precision symmetric matrix-matrix multiply:
    ...
    public static void Ssymm(
        ...
        float* b, int ldb,
        float* beta, float* c, int ldc) =>
        Check(_ssymm.Value(handle, side, uplo, m, n, alpha, a, lda, b, ldb, beta, c, ldc), "cublasSsymm_v2");

    /// <summary>
    /// Double-precision symmetric matrix-matrix multiply:
    /// C = alpha * A * B + beta * C (side=Left) or C = alpha * B * A + beta * C (side=Right)
    /// where A is symmetric.
    /// </summary>
    public static void Dsymm(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        int m, int n,
        double* alpha, double* a, int lda,
        double* b, int ldb,
        double* beta, double* c, int ldc) =>
        Check(_dsymm.Value(handle, side, uplo, m, n, alpha, a, lda, b, ldb, beta, c, ldc), "cublasDsymm_v2");
    ...
    /// </summary>
    public static void Strsm(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        CuBlasOperation trans, CuBlasDiagType diag,
        int m, int n,
        float* alpha, float* a, int lda,
        float* b, int ldb) =>
        Check(_strsm.Value(handle, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb), "cublasStrsm_v2");
    ...
    /// where A is triangular.
    /// </summary>
    public static void Dtrsm(
        nint handle, CuBlasSideMode side, CuBlasFillMode uplo,
        CuBlasOperation trans, CuBlasDiagType diag,
        int m, int n,
        double* alpha, double* a, int lda,
        double* b, int ldb) =>
        Check(_dtrsm.Value(handle, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb), "cublasDtrsm_v2");

    // ── Diagonal matrix multiply ──────────────────────────────────────────────

    /// <summary>
    /// Single-precision diagonal matrix multiply:
    /// C = diag(X) * A (side=Left) or C = A * diag(X) (side=Right).
    /// </summary>
    public static void Sdgmm(
        nint handle, CuBlasSideMode mode,
        int m, int n,
        float* a, int lda,
        float* x, int incx,
        float* c, int ldc) =>
        Check(_sdgmm.Value(handle, mode, m, n, a, lda, x, incx, c, ldc), "cublasSdgmm");

    /// <summary>
    /// Double-precision diagonal matrix multiply:
    /// C = diag(X) * A (side=Left) or C = A * diag(X) (side=Right).
    /// </summary>
    public static void Ddgmm(
        nint handle, CuBlasSideMode mode,
        int m, int n,
        double* a, int lda,
        double* x, int incx,
        double* c, int ldc) =>
        Check(_ddgmm.Value(handle, mode, m, n, a, lda, x, incx, c, ldc), "cublasDdgmm");
}
