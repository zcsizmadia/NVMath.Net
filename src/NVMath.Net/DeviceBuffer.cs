// Copyright (c) 2024 NVMath.Net Contributors. All rights reserved.
...
public sealed class DeviceBuffer<T> : IDisposable, IAsyncDisposable where T : unmanaged, INumberBase<T>
{
    ...
    /// </summary>
    public unsafe DeviceBuffer(long length)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(length);
        Length = length;
        _ptr = (nint)CudaRuntime.Malloc((nuint)(length * Unsafe.SizeOf<T>()));
    }
    ...
    public static unsafe DeviceBuffer<T> AllocAsync(long length, CudaStream stream)
    {
        ...
        buf._ptr = (nint)CudaRuntime.MallocAsync((nuint)(length * Unsafe.SizeOf<T>()), stream.Handle);
        return buf;
    }

    private DeviceBuffer(long length, bool allocate)
    {
        Length = length;
        _ptr = 0;
    }
